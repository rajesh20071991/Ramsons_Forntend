...
import { GetIndNonPolishData } from "../../redux/actions/inwardActions";
...
const Ind_NonPolish = () => {
  const Column_Col = [
    {
      ...
      dataIndex: "operation",
      render: (_, record) => {
        const editable = record.status === "Instock";
    ...
    },
    {
      ...
      dataIndex: "pipe_type",
      key: "pipe_type",
      sorter: (a, b) => a.pipe_type.localeCompare(b.pipe_type),
      ellipsis: true,
      width: "8%",
      showOnResponse: true,
      showOnDesktop: true,
    ...
    },
    {
      ...
      key: "no_of_pipe",
      sorter: (a, b) => a.no_of_pipe - b.no_of_pipe,
      ellipsis: true,
      showOnResponse: true,
      width: "7%",
      editable: true,
  ...
  var NonPolishPipe = useSelector(
    (state) => state.inwardData.inward_non_polish_data
  );

  if (NonPolishPipe) {
    var count = NonPolishPipe.count;
    NonPolishPipe = NonPolishPipe.results;
  }
  ...
  const [filter, setFilter] = useState(() => {
    const storedFilter = localStorage.getItem("pipeFilter");
    return storedFilter ? JSON.parse(storedFilter) : filtCols;
  });
  const [page, setPage] = useState(() => {
    const storedPage = localStorage.getItem("pipePage");
  ...
  useEffect(() => {
    ...
    api({ api: "/storeitem/companyinfilter/" })
      .then((data) => {
        setVendorNames(data);
      })
      .catch((error) => {
    ...
    api({ api: "/storeitem/pipethicknesslist/" })
      .then((data) => {
  ...
  useEffect(() => {
    localStorage.setItem("pipeFilter", JSON.stringify(filter));
  }, [filter]);

  useEffect(() => {
    localStorage.setItem("pipePage", page.toString());
  }, [page]);
  ...
  const columnsWithFilter = Column_Col.map((column) => {
    ...
    } else if (column.dataIndex === "pipe_type") {
      return {
        ...column,
        ...getColumnSearchProps(column.dataIndex),
      };
  ...
  console.log("columnsWithFilter:", columnsWithFilter); // Add this line to check the generated array
