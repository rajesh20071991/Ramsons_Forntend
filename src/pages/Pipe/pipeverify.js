...
import { GettubemillVerifyData } from "../../redux/actions/pipeAction";
...
const PipeNonPolishVerify_data = () => {
  const Column_Col = [
    {
      title: "Operation",
      dataIndex: "edit",
  ...
  const nonpolishpipe = useSelector(
    (state) => state.PipeData.TubemillVerify_Data
  );
  ...
  useEffect(() => {
    const storedFilter = localStorage.getItem("PipeVerifyFilter");
    const storedPage = localStorage.getItem("PipeVerifyPage");
    if (storedFilter) {
  ...
  useEffect(() => {
    localStorage.setItem("PipeVerifyFilter", JSON.stringify(filter));
  }, [filter]);

  useEffect(() => {
    localStorage.setItem("PipeVerifyPage", page.toString());
  }, [page]);
...
export default PipeNonPolishVerify_data;
