...
import { GetHolePipeData } from "../../redux/actions/pipeAction";
...
const HoleViewData = () => {
  const Column_Col = [
    ...
    {
      title: "Balance pipe",
      dataIndex: "balance_pipe",
      key: "balance_pipe",
      sorter: true,
      ellipsis: true,
      render: (text, record) => (record.pipe - record.pipe_out).toFixed(3),
    },
    {
      ...
      sorter: true,
      ellipsis: true,
      render: (text, record) => (record.Weight - record.weight_out).toFixed(3),
    },
  ];

  const holepipedata = useSelector((state) => state.PipeData.hole_pipe);

  if (holepipedata) {
    var count = holepipedata.count;
    var holepipe = holepipedata.results;
  }
