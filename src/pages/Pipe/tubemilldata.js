...
import { GettubemillPipeData } from "../../redux/actions/pipeAction";
import { SearchOutlined, FilterOutlined } from "@ant-design/icons";
import { SetModelId } from "../../redux/actions/modalAction";
import { Slitt_Coil_Form } from "./form/slitted";
...
const TubemillData = () => {
  const Column_Col = [
    ...
    },
    {
      ...
      ellipsis: true,
      render: (text, record) => {
        const pipe_out = record.pipe_out; // Access the 'pipe_out' value from the record
        return text - Number(pipe_out);
      },
    },
    {
      title: "Pipe Weight",
      ...
      ellipsis: true,
      render: (text, record) => {
        const weight_out = record.weight_out; // Access the 'pipe_out' value from the record
        return text - Number(weight_out);
      },
    },
    {
      title: "Burning Loss",
      dataIndex: "burning_loss",
      key: "burnLoss",
      sorter: true,
      ellipsis: true,
    },
  ...
  const nonpolishpipe = useSelector((state) => state.PipeData.tub_data);
