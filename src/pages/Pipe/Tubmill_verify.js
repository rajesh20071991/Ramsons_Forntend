...
import { GettubemillShiftData } from "../../redux/actions/pipeAction";
...
const TubemillVerify_data = () => {
  const Column_Col = [
    ...
    {
      title: "Scrap",
      dataIndex: "sum_scrap_weight",
      key: "sum_scrap_weight",
      sorter: true,
      ellipsis: true,
    },
    {
      title: "Hole Weight",
      dataIndex: "sum_hweight",
      key: "sum_hweight",
      sorter: true,
      ellipsis: true,
    },
    {
      title: "S/L Weight",
      dataIndex: "sum_short_weight",
      key: "sum_short_weight",
      sorter: true,
      ellipsis: true,
    },
    {
  ...
  const nonpolishpipe = useSelector(
    (state) => state.PipeData.TubemillShift_Data
  );
  ...
  useEffect(() => {
    const storedFilter = localStorage.getItem("nonPolishFilter");
    const storedPage = localStorage.getItem("nonPolishPage");
    if (storedFilter) {
  ...
  useEffect(() => {
    localStorage.setItem("nonPolishFilter", JSON.stringify(filter));
  }, [filter]);

  useEffect(() => {
    localStorage.setItem("nonPolishPage", page.toString());
  }, [page]);
  ...
  const columnsWithFilter = Column_Col.map((column) => {
    if (column.dataIndex && column.editable) {
      ...
      return {
        ...
        onCell: (record) => ({
          record,
          inputType: column.dataIndex === "weight_diff" ? "number" : "text",
    ...
    } else if (column.dataIndex === "crane_weight") {
      return {
        ...column,
        ...getColumnSearchProps(column.dataIndex),
      };
  ...
  function export_data() {
    ...
    let url = config.apiEndpoint + "/api/planlist/?token=" + key;
...
export default TubemillVerify_data;
