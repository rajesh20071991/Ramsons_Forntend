...
import { GetSalesCoilDispatch } from "../../redux/actions/dispatchAction";
...
const Coil_Sales = () => {
  ...
  function handleMenuClick(id, e) {
    ...
    switch (action) {
      case "pdfView":
        endpoint = `${EndPointApi}/dispatch/generatecoilview/pdf/${id}/0`;
        break;
      case "pdf":
        endpoint = `${EndPointApi}/dispatch/generatecoil/pdf/${id}/2`;
        break;
      case "email":
        endpoint = `${EndPointApi}/dispatch/generatecoil/pdf/${id}/1`;
        break;
      default:
        break;
    }
  ...
  const coilsDispatch = useSelector(
    (state) => state.DispatchData.Coil_sales_dispatch
  );

  if (coilsDispatch) {
    var count = coilsDispatch.count;
    var CoilSDispatch = coilsDispatch.results;
  }
  ...
  const { filter, setFilter, page, setPage } = useFilter("CoilSDispatchfilter");
  ...
  if (CoilSDispatch && Array.isArray(CoilSDispatch)) {
    planpipeWithExpandedData = CoilSDispatch.map((record) => ({
      ...record,
      expandedData:
        record.coils && Array.isArray(record.coils)
          ? record.coils.map((coiled) => ({
              size: coiled.size,
              coil: coiled.coil,
              stamp: coiled.stamp,
              grade: coiled.grade,
