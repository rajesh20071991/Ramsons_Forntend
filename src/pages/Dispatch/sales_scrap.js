...
import { GetScrapDatDispatch } from "../../redux/actions/dispatchAction";
...
const Scrap_Sales_Dispatch = () => {
  ...
  function handleMenuClick(id, e) {
    ...
    switch (action) {
      case "pdf":
        endpoint = `${EndPointApi}/dispatch/generatesscrap/pdf/${id}/0`;
        break;
      case "email":
        endpoint = `${EndPointApi}/dispatch/generatesscrap/pdf/${id}/1`;
        break;
      default:
        break;
    }
  ...
  const otherscrap = useSelector((state) => state.DispatchData.Coil_data);

  if (otherscrap) {
    var count = otherscrap.count;
    var OtherSDispatch = otherscrap.results;
  }
  ...
  if (OtherSDispatch && Array.isArray(OtherSDispatch)) {
    planpipeWithExpandedData = OtherSDispatch.map((record) => ({
      ...record,
      expandedData:
        record.other && Array.isArray(record.other)
          ? record.other.map((coiled) => ({
              grade: coiled.grade,
              rate: coiled.rate,
  ...
  function export_data() {
    ...
    let url =
      config.apiEndpoint +
      "/api/export/salesscrapdispatch/?from=" +
      from +
      "&to=" +
      to +
      "&token=" +
        ...
        <Form form={form} component={false}>
          {OtherSDispatch && (
            <Table
              ...
              scroll={{ x: true }}
              title={() => "Sales Scrap/Short Length"}
              components={{
                body: {
                  cell: EditableCell,
                },
...
export default Scrap_Sales_Dispatch;
