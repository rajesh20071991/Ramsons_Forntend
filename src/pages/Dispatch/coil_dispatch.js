...
import { GetCoilDispatchData } from "../../redux/actions/dispatchAction";
...
const Coil_Dispatch = () => {
  const Column_Col = [
    ...
    },
    {
      ...
      render: (_, record) => {
        ...
        const timeDifference = (currentTime - recordDate) / (1000 * 60 * 60); // Calculate time difference in hours
        if (timeDifference < 3) {
          return (
            <Popconfirm
  ...
  function handleMenuClick(id, e) {
    ...
    switch (action) {
      case "pdfView":
        endpoint = `${EndPointApi}/dispatch/generateview/pdf/${id}/0`;
        break;
      case "pdf":
        endpoint = `${EndPointApi}/dispatch/generatejob/pdf/${id}/0`;
        break;
      case "email":
        endpoint = `${EndPointApi}/dispatch/generatejob/pdf/${id}/1`;
        break;
      default:
        break;
    }
  ...
  const coilsDipatch = useSelector((state) => state.DispatchData.Coil_data);

  if (coilsDipatch) {
    var count = coilsDipatch.count;
    var CoilSDispatch = coilsDipatch.results;
  }
  ...
  const { filter, setFilter, page, setPage } = useFilter("coilsDipatchfilter");
  ...
  if (CoilSDispatch && Array.isArray(CoilSDispatch)) {
    planpipeWithExpandedData = CoilSDispatch.map((record) => ({
      ...record,
      expandedData:
        record.coil && Array.isArray(record.coil)
          ? record.coil.map((coiled) => ({
              coil: coiled.coil,
              width: coiled.width,
              grade: coiled.grade,
              ...
              challan_weight: coiled.challan_weight,
