...
import { GetdispatchProcessData } from "../../redux/actions/data_processAction";
...
const Dispatch_Process = () => {
  const Column_Col = [
    ...
    },
    {
      ...
      dataIndex: "challan_weight_sum",
      key: "challan_weight_sum",
      sorter: true,
    },
    {
      title: "Net Weight",
  ...
  var dispatchJwfms = useSelector(
    (state) => state.DataProcessData.dispatchProcess_data
  );
  if (dispatchJwfms) {
    var count = dispatchJwfms.count;
    var dispatchJwfms_data = dispatchJwfms.results;
  }
  ...
  const [filter, setFilter] = useState(() => {
    const storedFilter = localStorage.getItem("dispatchfmsFilter");
  ...
  const [page, setPage] = useState(() => {
    const storedPage = localStorage.getItem("dispatchfmsPage");
  ...
  useEffect(() => {
    localStorage.setItem("dispatchfmsFilter", JSON.stringify(filter));
  }, [filter]);

  useEffect(() => {
    localStorage.setItem("dispatchfmsPage", page.toString());
  }, [page]);
        ...
        <Form form={form} component={false}>
          {dispatchJwfms_data && (
            <Table
              ...
              bordered
              loading={dispatchJwfms_data === null}
...
export default Dispatch_Process;
