...
import { GetVehicleProcessData } from "../../redux/actions/data_processAction";
...
const Entry_Vehicle = () => {
  ...
  var vehiclefms = useSelector(
    (state) => state.DataProcessData.vehicleProcess_data
  );
  if (vehiclefms) {
    var count = vehiclefms.count;
    var vehiclefms_data = vehiclefms.results;
  }
  ...
  const [filter, setFilter] = useState(() => {
    const storedFilter = localStorage.getItem("vehiclefmsFilter");
  ...
  const [page, setPage] = useState(() => {
    const storedPage = localStorage.getItem("vehiclefmsPage");
  ...
  useEffect(() => {
    localStorage.setItem("vehiclefmsFilter", JSON.stringify(filter));
  }, [filter]);

  useEffect(() => {
    localStorage.setItem("vehiclefmsPage", page.toString());
  }, [page]);
  ...
  function handleSelectOption(option) {
    setSelectedOption(option.label); // Store only the label in the selectedOption state
  }

  const handlePostData = (e) => {
    e.preventDefault();
    // Here, you can perform the API call with the selected row data and selected option
    // using fetch, axios, or any other library

    console.log("Selected Row Data:", selectedRows);
    console.log("Selected Option:", selectedOption);
    api({
      api: "/api/entry_vehicle/",
      method: "post",
      body: { form: selectedRows, status: selectedOption, post: 1 },
    })
      .then(() => {
        ...
        >
          <div className="modal-content">
            <Select
              ...
              options={status.map((option) => ({ label: option.label }))} // Map the options array to include only the label
            />
          </div>
        </Modal>
      </div>
