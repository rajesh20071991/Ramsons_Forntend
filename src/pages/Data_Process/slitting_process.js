...
import { GetSlitProcessData } from "../../redux/actions/data_processAction";

const Slitting_Process = () => {
  const Column_Col = [
    {
      title: "Date",
  ...
  var slittingfms = useSelector(
    (state) => state.DataProcessData.slittingProcess_data
  );
  if (slittingfms) {
    var count = slittingfms.count;
    var slittingfms_data = slittingfms.results;
  }
  ...
  const [filter, setFilter] = useState(() => {
    const storedFilter = localStorage.getItem("slittingfmsFilter");
  ...
  const [page, setPage] = useState(() => {
    const storedPage = localStorage.getItem("slittingfmsPage");
...
export default Slitting_Process;
